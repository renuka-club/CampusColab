...
from .serializers import UserSerializer,TeacherSerializer,StudentSerializer,MessageSerializer,CourseSerializer,EventSerializer,StudentProfileSerializer
...
from .serializers import StudentCourseSerializer,CourseFeedSerializer,EventFeedSerializer
...
def get_feed_content(request):
    ...
    course_data = CourseFeedSerializer(courses, many=True).data
    event_data = EventFeedSerializer(events, many=True).data

    return Response({"courses": course_data, "events": event_data})
...
def add_student_course(request):
    ...
    try:
        ...
        if StudentCourse.objects.filter(student=student, course=course).exists():
            return Response({"message": "Course already added"}, status=status.HTTP_400_BAD_REQUEST)
...
@api_view(["GET"])
def get_student_courses(request, student_name):
    try:
        student = Students.objects.get(studentname=student_name)
...
class StudentProfileView(APIView):
    ...
    def post(self, request):
        ...
        student = get_object_or_404(Students, studentname=studentname)
...
class EventView(APIView):
    ...
    def post(self, request):
        try:
            ...
            if attachment:
                ...
                self.send_whatsapp_message(event_instance)
    ...
    def send_whatsapp_message(self, event):
        try:
            ...
            twilio_sid = os.getenv("TWILIO_ACCOUNT_SID", settings.TWILIO_ACCOUNT_SID)
            twilio_auth = os.getenv("TWILIO_AUTH_TOKEN", settings.TWILIO_AUTH_TOKEN)
            from_whatsapp = os.getenv("TWILIO_WHATSAPP_NUMBER", "whatsapp:+14155238886")  # Use Sandbox Number
            to_whatsapp = os.getenv("ADMIN_WHATSAPP_NUMBER", settings.ADMIN_WHATSAPP_NUMBER)
            ...
            message = client.messages.create(
                from_=from_whatsapp,  # Must be "whatsapp:+14155238886" in Sandbox mode
                body=message_body,
                to=to_whatsapp
            )
    ...
    def delete(self, request, eventname):
        ...
        try:
            ...
            events.delete()
            return Response({"message": f"✅ Deleted event: {eventname}"}, status=status.HTTP_204_NO_CONTENT)
...
class ProfileView(APIView):
    ...
    def post(self, request):
        ...
        if role == "Admin":
            ...
            try:
                user = Teachers.objects.get(teachername=name)
            ...
            try:
                user = Students.objects.get(studentname=name)
